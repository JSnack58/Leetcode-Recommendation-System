...
1. **User skill vs. problem difficulty** are the dominant signals. The top two features (overall_solve_rate and solve_rate) together account for roughly 68% of the total gain, confirming that the model primarily matches user ability level to problem difficulty.
...
```bash
# Train the model (reads raw JSONL data directly)
